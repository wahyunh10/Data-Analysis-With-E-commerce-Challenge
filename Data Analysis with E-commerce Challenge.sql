...
SELECT
    ...
    create_at AS tanggal_transaksi
FROM
    orders
WHERE
    buyer_id = 12476
...
WHERE
    create_at >= '2019-12-01'
    AND create_at < '2020-01-01'
    AND total >= 20000000
ORDER BY
    1;
...
SELECT
    ...
    COUNT(1) AS jumlah_transaksi,
    AVG(DATEDIFF(paid_at, create_at)) AS avg_lama_dibayar,
    MIN(DATEDIFF(paid_at, create_at)) min_lama_dibayar,
    MAX(DATEDIFF(paid_at, create_at)) max_lama_dibayar
FROM
    orders
WHERE
    paid_at IS NOT NULL
